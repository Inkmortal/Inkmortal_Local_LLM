...
const ChatInputFixed: React.FC<ChatInputProps> = ({ 
  onSend, 
  onStopGeneration,
  ...
  const [showGenerationBlocked, setShowGenerationBlocked] = useState(false);
  ...
  const generationBlockedTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  // Focus management
  useEffect(() => {
    if (textareaRef.current && !showMathEditor && !showCodeEditor) {
      textareaRef.current.focus();
    }
  }, [showMathEditor, showCodeEditor]);
  ...
  useEffect(() => {
    return () => {
      ...
      if (generationBlockedTimerRef.current) {
        clearTimeout(generationBlockedTimerRef.current);
      }
    };
  }, []);
  ...
  const handleSubmit = useCallback((e?: React.FormEvent) => {
    ...
    // Only allow non-empty messages that aren't during composition
    if (!message.trim() || isComposing) {
      return;
    }
    
    // If generating, show feedback but don't clear the message
    if (isGenerating) {
      // Show a temporary indicator that sending is blocked
      setShowGenerationBlocked(true);
      ...
      generationBlockedTimerRef.current = setTimeout(() => {
        setShowGenerationBlocked(false);
        generationBlockedTimerRef.current = null;
      }, 2000);
    ...
    <div className="w-full relative z-50">
      ...
      >
        <div 
          ...
          } ${isTyping ? 'is-typing' : ''} ${showGenerationBlocked ? 'ring-red-500' : ''}`}
          style={{ 
        ...
        >
          ...
          <div 
            ...
            style={{
              background: showGenerationBlocked 
                ? 'linear-gradient(to right, #ef4444, #dc2626)'
                : `linear-gradient(to right, ${currentTheme.colors.accentPrimary}, ${currentTheme.colors.accentSecondary}, ${currentTheme.colors.accentPrimary})`,
              backgroundSize: '200% 100%',
              animation: isFocused ? 'gradientAnimation 6s ease infinite' : 'none'
            }}
          />
          
          {/* Generation blocked message */}
          {showGenerationBlocked && (
            <div className="absolute top-0 left-0 right-0 text-xs text-center py-1 bg-red-500 text-white">
              Please wait until the AI finishes generating its response
            </div>
          )}
          ...
          <textarea 
            ...
            disabled={false} // Never disable the textarea to allow typing during generation
            placeholder={isGenerating ? "AI is generating..." : placeholder}
            rows={1}
            className={`w-full px-4 pt-3.5 pb-2 resize-none overflow-auto focus:outline-none z-10 relative ${showGenerationBlocked ? 'pt-6' : ''}`}
            style={{ 
          ...
          <div className="px-3 pb-2.5 flex justify-between items-center relative z-10">
            ...
            {/* The button changes between Send and Stop based on generation state */}
            {isGenerating ? (
              <Button
                type="button"
                onClick={onStopGeneration}
                size="sm"
                className="rounded-full py-2 px-4 transition-all hover:scale-[1.03]"
                style={{ 
