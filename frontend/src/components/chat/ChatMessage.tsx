...
const ChatMessage: React.FC<ChatMessageProps> = ({ 
  ...
  const isQueued = message.status === MessageStatus.QUEUED;
  const isProcessing = message.status === MessageStatus.PROCESSING;
  const isComplete = message.status === MessageStatus.COMPLETE || !message.status;
  ...
  // Display relevant status indicator
  const getStatusIndicator = () => {
    if (isUser) {
      if (isSending) return "Sending...";
      if (isQueued) return message.queue_position ? `In queue (pos: ${message.queue_position})` : "In queue...";
      if (hasError) return "Error sending message";
      if (isProcessing) return "Processing...";
      return null; // No indicator for complete user messages
    } else if (isAssistant) {
      if (isGenerating) return "Generating...";
      if (hasError) return "Error generating response";
      return null; // No indicator for complete assistant messages
    } else if (isSystem) {
      return null; // System messages don't need status indicators
    }
  };
    ...
    >
      ...
      {isUser && (
        <div className="w-9 h-9 rounded-full flex-shrink-0 ml-3 overflow-hidden relative shadow-lg order-2">
          ...
          {/* Status indicator for user messages being processed */}
          {(isSending || isQueued || isProcessing) && (
            <div
              className="absolute inset-0 rounded-full z-20"
              style={{
                background: `conic-gradient(transparent, transparent, ${currentTheme.colors.accentSecondary})`,
                animation: 'spin 1.5s linear infinite',
              }}
            />
          )}
      ...
      <div 
        ...
        style={{ 
          ...
          border: hasError
            ? `1px solid ${currentTheme.colors.error}50`
            : isAssistant 
      ...
      >
        {/* Status indicator for messages in progress */}
        {(isSending || isQueued || isProcessing || isGenerating) && (
          <div className="absolute top-0 left-0 right-0 h-1 rounded-t-lg overflow-hidden z-20">
            <div 
              className="h-full" 
              style={{
        ...
        {/* Error indicator highlight for error messages */}
        {hasError && (
          <div 
        ...
        {isAssistant && !hasError && !(isSending || isQueued || isProcessing || isGenerating) && (
          <div 
            className="absolute top-0 left-0 right-0 h-1 rounded-t-lg"
            style={{
        ...
        {isUser && !hasError && !(isSending || isQueued || isProcessing) && (
          <div 
            className="absolute top-0 left-0 right-0 h-1 rounded-t-lg"
            style={{
              background: `linear-gradient(to right, ${currentTheme.colors.borderColor}50, ${currentTheme.colors.borderColor}50)`,
        ...
        <div 
          className="text-xs mt-2 flex justify-between items-center relative z-10"
          style={{ 
            color: hasError 
        ...
        >
          ...
          <div className={`status-text ${getStatusIndicator() ? 'opacity-100' : 'opacity-0'}`}>
            {getStatusIndicator()}
          </div>
          
          {/* Timestamp on the right */}
          <div className="timestamp opacity-70">
            {getTimeDisplay()}
          </div>
        </div>
        ...
        {hasError && message.error && (
          <div 
            className="text-xs mt-2 relative z-10 p-2 rounded"
            style={{ 
