...
// Optimize memoization to allow streaming updates to trigger re-renders
export default memo(ChatWindowFixed, (prevProps, nextProps) => {
  ...
  // Check if any message content has changed - critical for streaming!
  for (let i = 0; i < prevProps.messages.length; i++) {
    const prevMsg = prevProps.messages[i];
    const nextMsg = nextProps.messages[i];
    
    // Skip ID comparison and go straight to content comparison for efficiency
    // During streaming, the ID stays the same but content changes
    if (prevMsg.content !== nextMsg.content) return false;
    if (prevMsg.status !== nextMsg.status) return false;
    
    // Also check sections if they exist (for thinking/reasoning sections)
    if (prevMsg.sections?.response?.content !== nextMsg.sections?.response?.content) return false;
    if (prevMsg.sections?.thinking?.content !== nextMsg.sections?.thinking?.content) return false;
  }
  
  // Messages haven't changed, no need to re-render
  return true;
});