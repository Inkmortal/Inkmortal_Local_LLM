...
import { fetchApi, fetchAdminSetup, checkBackendConnection } from '../../config/api';
...
const AdminLogin: React.FC = () => {
    ...
    const initialize = async () => {
      ...
      try {
        ...
        try {
          ...
          // Use the specialized non-authenticated function
          const response = await fetchAdminSetup('/auth/admin/setup-status');
  ...
  const fetchSetupToken = async () => {
    try {
      ...
      // Use the specialized function for admin setup that doesn't require auth
      const response = await fetchAdminSetup('/auth/admin/fetch-setup-token');
  ...
  const handleSetup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use specialized function for admin setup that bypasses auth checks
      const response = await fetchAdminSetup('/auth/admin/setup', {
        method: 'POST',
        headers: {
